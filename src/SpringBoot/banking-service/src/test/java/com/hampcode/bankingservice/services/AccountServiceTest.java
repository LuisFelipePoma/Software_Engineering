...
public class AccountServiceTest {
	...
	public void testCreateAccount() {
		...
		when(accountMapper.convertToDTO(account)).thenReturn(new AccountResponseDTO(account.getId(),
				account.getAccountNumber(), account.getBalance(), account.getOwnerName(), account.getOwnerEmail()));
		...
		// Verificamos que el método convertToEntity del mock accountMapper se haya
		// llamado una vez con el requestDTO
		verify(accountMapper, times(1)).convertToEntity(requestDTO);
		// Verificamos que el método save del mock accountRepository se haya llamado una
		// vez con el account
		verify(accountRepository, times(1)).save(account);
		// Verificamos que el método convertToDTO del mock accountMapper se haya llamado
		// una vez con el account
